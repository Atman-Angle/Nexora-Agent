...
Both files import only `@nexora/runtime`. The D5 package tests copy them into
independent temporary projects, install the same `pnpm pack` tarball, compile
them with strict NodeNext TypeScript, and execute them outside the monorepo.

The HTTP routes are an example application's contract. They are not exported
by `@nexora/runtime` and are not a Nexora remote-runtime or application
framework API.
