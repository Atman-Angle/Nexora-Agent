...
- `qwen3.7-flash` 不再使用特殊的 180 秒默认等待，回到通用 60 秒；显式 `NEXORA_MODEL_TIMEOUT_MS` 仍优先；
...
8. qwen 默认 timeout 为 60 秒，显式 timeout 保持优先；
...
本地证据：Provider/Desktop targeted 19/19、Context Quality 65/65、Runtime/Harness release 88/88、typecheck、lint、build 和 deterministic Electron UAT 均通过。真实 Provider UAT 未执行；目标 Session 的历史记录确认两次 180 秒 Provider timeout，旧失败 Attempt 的临时 reasoning 被误当成持续进度显示，后续另有一个进程退出后遗留的 `started` Attempt。当前实现将 qwen 默认 timeout 收敛为 60 秒、按既有三次 Attempt 审计重试，并且只持久化成功 Attempt transcript。成功 transcript 可能包含模型已返回的 Workspace 内容，继续受本地 Artifact 保留、磁盘加密和访问控制等部署 Release Gate 约束。
