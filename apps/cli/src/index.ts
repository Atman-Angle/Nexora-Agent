...
function parseSuccessCriteriaEnv(value: string | undefined): string[] { const normalized = value?.trim(); if (normalized === undefined || normalized.length === 0) return []; const parsed: unknown = JSON.parse(normalized); if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string" || item.length === 0)) throw new InvalidInputError("NEXORA_AGENT_SUCCESS_CRITERIA_JSON must be a JSON array of non-empty strings."); return parsed; }
