...
| **Manual and automatic compaction** | A terminal Run can append an idempotent `context.compaction.requested` event; the next continuation projects older ancestors as references and the requested boundary as compact, without deleting history or calling a summarizer. Independently, measured Provider token pressure triggers deterministic automatic eviction, recorded on the persisted `model.requested` event. |
...
Desktop is a two-column Runtime host: Projects (Workspaces) and Sessions on the left, and one Conversation/Activity execution surface in the center. OpenAI-compatible `native_tools` Providers can stream explicitly returned `content` and `reasoning_content`; reasoning stays in a compact two-line Think view that expands inline, and successful Attempt transcripts remain available after reopening the Session. Failed or timed-out Attempt text is discarded. Provider liveness separates a one-minute response-header timeout, a five-minute stream-idle timeout renewed by every real SSE frame, and a 30-minute Attempt ceiling, so active reasoning is not cut off by a short fixed request timer. Tool names and status stay compact with a bounded real result preview, while a click reveals the full Invocation details. Tool, Evidence, Validation, and completion still come from Runtime authority; public output and Results render safe Markdown. Results expose deduplicated links for successful Workspace file outputs, opened through a Project-bounded Desktop Host bridge. Enter sends, Shift+Enter adds a line, and the Composer remains available while a Run is active and after it terminates. Enter `/压缩上下文` (or `/compact`) to safely stop an active Run if needed and append a Runtime-owned compaction request for the next turn; the command itself is not sent to the model. Settings manages one global Provider/model catalog, including multiple models per Provider; each Project selects the profile used by future Runs without interrupting active Runs. One Desktop installation can manage any added local Workspace.
