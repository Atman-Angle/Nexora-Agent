...
export const DECISION_PROMPT_LAYERS = Object.freeze({
  ...
  phase: `Return exactly one JSON object matching an example in context.intentContract, with optional reasoningSummary and one intent. Use only a kind listed in context.allowedIntents. plan_tasks contains semantic Task Contract fields plus ordered tasks and semantic completionRequirements; Runtime preserves completed tasks and creates all internal IDs. use_capabilities contains only registered capability names and complete business arguments; Runtime binds and batches them. finish contains only a verified user-facing summary.`,
  task: `Preserve every explicit user action, constraint, ordering requirement and acceptance condition. For an explicit Memory or History restoration requirement, include a context_ref completionRequirement with the exact published ref. If acceptance is ambiguous, use request_input rather than inventing a requirement. Revise only the unfinished semantic tasks; never copy the current internal Plan.`,
